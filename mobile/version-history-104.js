const VERSIONS = [
  {
    ...
    changes: [
      ...
      "Удалён зацикленный MutationObserver, который бесконечно изменял DOM и блокировал завершение запуска Firebase.",
      "Загрузка приложения больше не ждёт ответа всех Firestore-подписок: после авторизации основной экран открывается сразу, а данные догружаются внутри интерфейса.",
      "Сохранены исправления склада: в «Операциях» нет кнопки «+ Продажа», журнал движения открывается только отдельной кнопкой «Журнал», разновидности выводятся вертикально с цветными кружками.",
      "Добавлена защита от повторного зависания стартового экрана при задержке сети или отдельных коллекций Firestore."
    ]
  },
  {
    version: "1.0.3",
    date: "06.09.2026",
    changes: [
      "Критические правки интерфейса перенесены в основную HTML-разметку.",
...
function ensureHistoryUi() {
  ...
  if (!button) {
    ...
    button.innerHTML = '<span class="version-history-btn-copy"><b>История изменений</b><small>Актуальная версия: 1.0.4</small></span><span aria-hidden="true">›</span>';
  ...
  renderHistory();
  if (button.dataset.bound104 !== "1") {
    button.dataset.bound104 = "1";
